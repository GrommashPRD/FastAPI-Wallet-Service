...
from app.repository import exceptions as repo_exceptions
        ...
        if check_conditions:
            existing_record_stmt = select(cls.model).where(
                and_(
                    getattr(
                        cls.model, key
                    ) == value for key, value in check_conditions.items()
                )
            )
            ...
            if existing_record.scalars().first():
                raise repo_exceptions.RecordAlreadyExist("Record already exists.")
